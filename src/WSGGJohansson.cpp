...
// double WSGGJohansson::kappa(int igas, double pH2O, double pCO2){
...
vector<vector<double>> WSGGJohansson::solve(vector<double> mu, int NPoints, vector <double> T, vector <double> P, vector <double> XH2O, vector <double> XCO2, vector <double> DeltaX, string sign){
	...
	for(int muj=0; muj<mu.size(); muj++){
		...
		for(int i=0; i<NPoints; i++){
			...
			for(int iBand =1; iBand<NbBands(); iBand++){
				...
				// kappa_coeffs[iBand] = kappa(iBand-1,pH2O,pCO2);
				// if (i == 0){
				// 	MR = 2.0;
				// }
				// else{ 
				...
				kappa_coeffs[iBand] = kappa(iBand-1, XH2O[i], XCO2[i]);
				// }
				acoeffs[iBand] = a(iBand-1, T[i]/1200., MR);
				sum_of_coeffs += acoeffs[iBand];
			}
			...
			for(int iBand =0; iBand<NbBands(); iBand++){
				...
				if(i==0){
					...
					exp(-kappa_coeffs[iBand] * (XH2O[i]+XCO2[i]) * 0.986923 * (P[i]) * DeltaX[i-1]/mu[muj]); 
