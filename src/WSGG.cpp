...
double WSGG::a(int iBand, double T){
    ...
    return (b[iBand][2]*pow(T,0)*1e-0 + b[iBand][4]*pow(T,1) + b[iBand][6]*pow(T,2) + b[iBand][8]*pow(T,3) + b[iBand][10]*pow(T,4))*
                     (b[iBand][3]*pow(T,0)*1e-0 + b[iBand][5]*pow(T,1) + b[iBand][7]*pow(T,2) + b[iBand][9]*pow(T,3) + b[iBand][11]*pow(T,4));
}
...
vector<vector<double>> WSGG::solve(vector<double> mu, int NPoints, vector <double> T, vector <double> P, vector <double> XH2O, vector <double> XCO2, vector <double> DeltaX, string sign){
        ...
        for(int i=0; i<NPoints; i++){
            ...
            for(int iBand =0; iBand<NbBands(); iBand++){
                ...
                if(i==0){
                    ...
                    I[muj][iBand][i] = a(iBand,T[i])*BlackBodyIntensity(T[i]) + (I[muj][iBand][i-1] -a(iBand,T[i])*BlackBodyIntensity(T[i]))*exp(-kappa(iBand,pH2O,pCO2)*DeltaX[i-1]/mu[muj]); 
